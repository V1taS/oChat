...
public struct ChatView<MessageContent: View, InputViewContent: View>: View {
  ...
  var showMessageName: Bool = false
  ...
  func onMessageMenuAction(row: MessageRow, action: MessageMenuAction) {
    ...
    case .retry:
      row.message.retryAction?(row.message)
    case .delete:
      row.message.deleteAction?(row.message)
    case .copy:
      row.message.copyAction?(row.message)
    }
  }
}
...
public extension ChatView {
  ...
  func showMessageName(_ show: Bool) -> ChatView {
    var view = self
    view.showMessageName = show
    return view
  }
