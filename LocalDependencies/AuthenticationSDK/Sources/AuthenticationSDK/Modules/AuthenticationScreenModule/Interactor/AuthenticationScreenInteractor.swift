...
final class AuthenticationScreenInteractor {
  ...
  private let messengerModelHandlerService: IMessengerModelHandlerService
  private let messengerAppSettingsManager: IAppSettingsManager
  ...
  init(_ services: IApplicationServices) {
    ...
    messengerModelHandlerService = services.messengerService.modelHandlerService
    messengerAppSettingsManager = services.messengerService.appSettingsManager
  }
}
...
extension AuthenticationScreenInteractor: AuthenticationScreenInteractorInput {
  func setAccessCode(_ code: String, completion: (() -> Void)?) {
    ...
    messengerAppSettingsManager.setAppPassword(code, completion: completion)
  }
  
  func getIsFaceIDEnabled(completion: ((Bool) -> Void)?) {
    modelHandlerService.getAppSettingsModel { [weak self] _ in
      guard let self else { return }
      messengerModelHandlerService.getAppSettingsModel { appSettingsModel in
