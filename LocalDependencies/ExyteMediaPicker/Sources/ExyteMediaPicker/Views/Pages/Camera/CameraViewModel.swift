...
final class CameraViewModel: NSObject, ObservableObject {
  ...
  func flipCamera() async {
    sessionQueue.async { [weak self] in
      ...
      DispatchQueue.main.async { [weak self] in
        ...
        // Удаляем текущий вход
        session.removeInput(currentInput)
        
        // Находим новую камеру и добавляем её
        if let newDevice = self?.selectCaptureDevice(for: newPosition),
           let newInput = try? AVCaptureDeviceInput(device: newDevice) {
          if session.canAddInput(newInput) {
            session.addInput(newInput)
            self?.updateCaptureDevice(with: newDevice)
          }
        }
  ...
  private func updateCaptureDevice(with device: AVCaptureDevice) {
