...
struct StandardConrolsCameraView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      HStack {
        Button(ExyteMediaPickerStrings.cameraButtonCancelTitle) {
          if cameraSelectionService.hasSelected {
            viewModel.showingExitCameraConfirmation = true
          } else {
            didPressCancel()
      ...
      VStack(spacing: 10) {
        if cameraSelectionService.hasSelected {
          HStack {
            Button(ExyteMediaPickerStrings.cameraButtonDoneTitle) {
              if cameraSelectionService.hasSelected {
  ...
  var photoVideoToggle: some View {
    HStack {
      Button(ExyteMediaPickerStrings.cameraButtonVideoTitle) {
        capturingPhotos = false
        impactFeedback.impactOccurred()
      }
      .foregroundColor(capturingPhotos ? Color.white : Color.yellow)
      
      Button(ExyteMediaPickerStrings.cameraButtonPhotoTitle) {
